...
addBiasToTraits <- function(x, colB = NULL, SD = 0.001, traceB = FALSE) {
  ...
  temp.df <- x[, !colnames(x) %in% "Taxa"] %>%
    group_by_all() %>%
    count() %>%
    filter(n > 1)

  if (nrow(temp.df) == 0) {
    message("No duplicated traits")
    return(x)
  } else {
    message("Duplicated traits detected")
    ...
    for (i in 1:nrow(temp.df)) {
      to.sel <- apply(x[, !colnames(x) %in% "Taxa"], 1, function(z) identical(z, t(temp.df[i, -ncol(temp.df)])[, 1]))
      res.names <- which(to.sel)
