...
dness <- function(d, complete = F, taxLev = "Genus", method = "delta", taxatree = F) {
  ...
  exclude <- -which(names(tree) %in% c("Taxa", "Subclass", "Subfamily", "Tribus", "Subspecies"))
  tree <- tree[ , exclude, drop = F]
  ...
  if(complete == F & che.ck == F){
    ...
    taxtree <- merge(ref, df, by = "Taxa", all = F)
