...
psi <- function(x, method = "extence", abucl = c(1, 9, 99, 999), agg = FALSE, fssr_scores = NULL, exceptions = NULL, traceB = FALSE) {
  ...
  res.tot <- tot.mer %>%
    pivot_longer(-c(Taxon, FSSR_Score), names_to = "Sample", values_to = "ABUCLASS") %>%
    dplyr::rename("FSSR" = "FSSR_Score") %>%
    inner_join(w, by = c("FSSR", "ABUCLASS")) %>%
    select(c("Sample", "SCORE")) %>%
    group_by(Sample) %>%
    summarise(PSI = sum(SCORE)) %>%
    deframe()
  ...
  if (traceB == FALSE) {
    ...
    temp <- tot.mer %>%
      ...
      dplyr::rename("FSSR" = "FSSR_Score") %>%
      inner_join(w, by = c("FSSR", "ABUCLASS")) %>%
      select(c("Taxon", "Sample", "SCORE")) %>%
      pivot_wider(names_from = Sample, values_from = SCORE, values_fill = list(SCORE = 0)) %>%
      as.data.frame()
