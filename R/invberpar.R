...
invberpar <- function( x , tax_lev = "Taxa" ){
  ...
  # apply the function for calculating the inverse Berger-Parker that is stored in the Pi.R file
  res <- apply( DF[ , -1 , drop = FALSE ] , 2 , FUN = function( x ){ Pi( x , index = "Invberpar" ) } )
  res
}
