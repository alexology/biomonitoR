...
#' Please modify them if you know what you are doing.
#'
...
#'
#' f_eve( data.agR , traitDB = data.ts.av , type = "F" , col_blocks = col_blocks )
#' f_eve( data.agR , traitDB = data.ts.av , type = "F" , col_blocks = col_blocks ,
...
#'
#' f_eve( data.agR , traitDB = traits.dist   )
#'
#'
#' @seealso [aggregatoR]
#'
...
f_eve <- function( x , traitDB = NULL, taxLev = "Taxa" , type = NULL , traitSel = FALSE , col_blocks = NULL,  nbdim = 2 , distance = "gower", zerodist_rm = FALSE ,  correction = "none" , traceB = FALSE , set_param = NULL ){
  ...
  # transpose DF to fit with the feve_k

  abu.t <- t( DF[ , -1 ] )
  colnames( abu.t ) <- DF$Taxon

  res <- feve_k( fpc = traits_pcoa$li, taxa = abu.t , m )
