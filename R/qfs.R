...
qfs <- function(dist_funct, nbdim = nbdim, plot = "quality_funct_space") {
  ...
  # checking data
  if (!("dist" %in% class(dist_funct))) {
    stop(" 'dist_funct' must be of class 'dist'")
  }
  if (length(dist_funct) < 3) {
    stop(" there must be at least 3 species in 'dist_funct' ")
  }
  if (sum(is.na(dist_funct)) != 0) {
    stop(" NA are not allowed in 'dist_funct' ")
  }
  if (nbdim < 2) {
    stop(" 'nbdim' must be higher than 1")
  }
...
fdisp_k <- function(d, a, m) {
  ...
  if (any(sn.d != sn.a)) {
    stop(
      "Species labels in 'd' and 'a' need to be identical and ordered alphabetically (or simply in the same order).",
      "\n"
    )
  }
  a[which(is.na(a))] <- 0
  ...
  if (any(abun.sum == 0)) {
    warning(
      "At least one community has zero-sum abundances (no species).",
      "\n"
    )
  }
  ...
  vectors <- vectors[, 1:r, drop = FALSE] %*% diag(
    sqrt(abs(eig <- eig[1:r])),
    r
  )
...
feve_k <- function(fpc, taxa, m) {
  ...
  for (spp in 1:tax.pool) {
    paste(rep("spp", rdt), spp, sep = "") -> spp.list[((spp - 1) * rdt + 1):(spp * rdt)]
  }

  colnames(taxa.rep) <- spp.list

  for (spp in 1:tax.pool) {
    taxa.rep[, ((spp - 1) * rdt + 1):(spp * rdt)] <- taxa[, spp] / rdt
  }
