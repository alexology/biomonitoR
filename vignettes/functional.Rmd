...
f_disp( data.agR , traitDB = data.ts.av , nbdim = 2 , type = "F" , col_blocks = col_blocks )
...
f_rich( data.agR , traitDB = data.ts.av , nbdim = 2 , type = "F" , col_blocks = col_blocks )
```
...
```{r traceB }
frich <- f_rich( data.agR , traitDB = data.ts.av , nbdim = 2 , type = "F" , col_blocks = col_blocks , traceB = TRUE )
...
# functional richness
f_rich( data.agR , traitDB = data_fam.ts.av , taxLev = "Family" , nbdim = 2 , type = "F" , col_blocks = col_blocks )
...
oglio.asb <- asBiomonitor( oglio , group = "mf" , FUN = bin )
oglio.agg <- aggregatoR( oglio.asb )

# no need to run traitScaling, just to show its use
oglio.ts <- traitScaling( oglio.agg , traitDB = traits_mf , group = "mf" )
oglio.ts[ 1:5 , 1:5 ]
...
selectPcoaAxes( oglio.dist , method = "legendre" )

# calculate functional richness
f_rich( oglio.agg , traitDB = oglio.dist , taxLev = "Taxa" , nbdim = 4 , correction = "quasi" )
